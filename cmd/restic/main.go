...
var (
	...
	Command = &cli.Command{
		...
		Flags: []cli.Flag{
			...
			&cli.StringFlag{Destination: &cfg.Config.ResticFlags, Name: "resticFlags", EnvVars: []string{"RESTIC_FLAGS"}, Usage: "Additional flags to pass to restic in the format '--flag1=value;--flag2=value2"},
			&cli.StringFlag{Destination: &cfg.Config.ResticIgnoreFile, Name: "resticIgnoreFile", EnvVars: []string{"RESTIC_IGNORE_FILE"}, Usage: "Filename of an ignore file that will be included with --exclude-file. Needs to be present in the root dir of the pv"},
