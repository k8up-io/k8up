...
func connectToS3Server(t *testing.T, ctx context.Context) *s3.Client {
	repo := getS3Repo()
	s3client := s3.New(repo, os.Getenv("AWS_ACCESS_KEY_ID"), os.Getenv("AWS_SECRET_ACCESS_KEY"), os.Getenv("AWS_REGION"))
...
func testCheckS3Restore(t *testing.T, ctx context.Context) {
	s3c := s3.New(os.Getenv("RESTORE_S3ENDPOINT"), os.Getenv("RESTORE_ACCESSKEYID"), os.Getenv("RESTORE_SECRETACCESSKEY"), os.Getenv("RESTORE_REGION"))
	err := s3c.Connect(ctx)
	require.NoError(t, err)
