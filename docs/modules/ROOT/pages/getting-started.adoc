...
helm install appuio/k8up --namespace k8up-operator --generate-name
----
...
Additionally the operator exposes a `:8080/metrics` endpoint for prometheus scraping. This will give you additional metrics that can be used to find failed jobs. See the (https://github.com/vshn/k8up/tree/master/manifest/examples/prometheus)[Prometheus examples] in our Github repository.
