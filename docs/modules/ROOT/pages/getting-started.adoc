...
The default MinIO installation uses the access key `minio` and secret key `minio123`. They are in plain text inside the `backup-credentials` Secret definition and will be encoded as Base64 when the Secret is created on your cluster.
