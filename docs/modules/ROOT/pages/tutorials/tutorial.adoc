...
The restore job restores data into an existing but empty PVC mounted in the WordPress pod under `/restore`:
...
- name: wordpress-restore-storage
  mountPath: /restore
....

This is clearly not what we need for a full restoration; we need to move the files to their correct location.
...
[start=2]
. Replace the files inside the pod with the ones from the restore. Using rsync here simplifies things by only copying files that are different from the target and also deleting any additional malicious files an attacker may have added:

....
# apt update && apt install rsync
# rsync -a --delete /restore/ /var/www/html/
....
