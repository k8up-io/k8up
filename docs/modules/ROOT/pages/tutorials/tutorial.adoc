...
. Replace the files inside the pod with the ones from the restore. 
  Using rsync here simplifies things by only copying files that are different from the target and also deleting any additional malicious files an attacker may have added:
